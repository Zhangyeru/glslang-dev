...
vec4 loadBias(int col0)
{
    return vec4(Bias.data[col0 + 0], Bias.data[col0 + 1], Bias.data[col0 + 2], Bias.data[col0 + 3]);
}
...
void main()
{
    ...
    for (int col0 = 0; col0 < 16; col0 += 4)
    {
        ...
        int param_4 = col0;
        int param_5 = col0;
        vec4 param_6 = loadBias(param_4) + vec4(((rx.x + rx.y) + rx.z) + rx.w, ((ry.x + ry.y) + ry.z) + ry.w, ((rz.x + rz.y) + rz.z) + rz.w, ((rw.x + rw.y) + rw.z) + rw.w);
