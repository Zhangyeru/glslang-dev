...
- SPIR-V tools binaries live at `build/External/spirv-tools/tools/` — use `spirv-as`, `spirv-dis`, `spirv-opt`, `spirv-val`, `spirv-link`, `spirv-reduce` from there (e.g. `build/External/spirv-tools/tools/spirv-val out.spv`).
